# shady-fe-eslint-config

> shady JavaScript TypeScript Node 规范

提供了多套配置文件以支持 `JavaScript`、`TypeScript`、`React`、`Vue`、`Node.js` 等多种项目类型。

## JavaScript 项目 - shady-fe-eslint-config
...
```shell
npm i -D shady-fe-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import
```
...
```json
{
  "extends": ["shady-fe-eslint-config"]
}
```

## JavaScript + React 项目 - shady-fe-eslint-config/react
...
```shell
npm i -D shady-fe-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import eslint-plugin-react eslint-plugin-react-hooks
```
...
```json
{
  "extends": ["shady-fe-eslint-config/react"]
}
```
...
```json
{
  "extends": ["shady-fe-eslint-config/react", "shady-fe-eslint-config/jsx-a11y"]
}
```

## JavaScript + Vue 项目 - shady-fe-eslint-config/vue
...
```shell
npm i -D shady-fe-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import vue-eslint-parser eslint-plugin-vue
```
...
```json
{
  "extends": ["shady-fe-eslint-config/vue"]
}
```

## JavaScript (Node.js) 项目 - shady-fe-eslint-config/node
...
```shell
npm i -D shady-fe-eslint-config @babel/core @babel/eslint-parser eslint-plugin-import eslint-config-egg
```
...
```json
{
  "extends": ["shady-fe-eslint-config/node"]
}
```

## TypeScript 项目 - shady-fe-eslint-config/typescript
...
```shell
npm i -D shady-fe-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript"]
}
```
...
## TypeScript + React 项目 - shady-fe-eslint-config/typescript/react
...
```
npm i -D shady-fe-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-plugin-react eslint-plugin-react-hooks
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript/react"]
}
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript/react", "shady-fe-eslint-config/jsx-a11y"]
}
```

## TypeScript + Vue 项目 - shady-fe-eslint-config/typescript/vue
...
```shell
npm i -D shady-fe-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript vue-eslint-parser eslint-plugin-vue
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript/vue"]
}
```

## TypeScript (Node.js) 项目 - shady-fe-eslint-config/typescript/node
...
```
npm i -D shady-fe-eslint-config @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-config-egg
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript/node"]
}
```
...
```json
{
  "extends": ["shady-fe-eslint-config/typescript/react", "prettier"]
}
```
...
为了保证一致的编码风格，本包中大量风格相关的规则被设为了 `error` 级别，以引起开发者的足够重视。如果你觉得风格问题不足以是 `error` 级别（有些用户根据 ESLint error 进行流程卡点），本包还提供了一套名为 'essential' 的配置文件，这套配置将所有风格问题降级为 `warn` 级别，仅将必要问题报告为 `error`，引用方式为在相应配置的 `shady-fe-eslint-config` 后面加上 `/essential`，如对 `JS React` 项目为 `shady-fe-eslint-config/essential/react`、对 `TS Vue` 项目为 `shady-fe-eslint-config/essential/typescript/vue`
...
- 了解如何在继承本包的基础上对项目 ESLint 进行个性化配置，可参考官网的 [Configuring ESLint](https://eslint.org/docs/user-guide/configuring)。下面简介下 ESLint 配置中的几个常用字段：
  - `extends`: 继承一组规则集。`"extends": "shady-fe-eslint-config",` 表示继承本包定义的规则配置。
