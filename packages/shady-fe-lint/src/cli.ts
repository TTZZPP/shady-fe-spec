...
const installDepsIfThereNo = async () => {
  const lintConfigFiles = [].concat(
    glob.sync('eslint.config?(.@(js|mjs|cjs|ts|mts|cts))', { cwd }),
...
program
  .version(PKG_VERSION)
  .description(
    `${PKG_NAME} 是 SHADY前端编码规范工程化 的配套 Lint 工具，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡点，降低项目实施规范的成本`
  );
