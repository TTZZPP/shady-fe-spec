 ...
 * @param cwd
 * @param pkg
 * @returns shady-fe-eslint-config/index
 * @returns shady-fe-eslint-config/react
 * @returns shady-fe-eslint-config/typescript/index
 * @returns shady-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'shady-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
